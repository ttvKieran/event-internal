...
CONNECTOR_NAME=${CONNECTOR_NAME:-analytics-outbox-connector}
DB_HOST=${DB_HOST:-analytics-db}
...
ROUTE_TOPIC_REPLACEMENT=${ROUTE_TOPIC_REPLACEMENT:-Metrics}
...
echo "Đăng ký Connector cho Analytics Service..."
...
curl -i -X POST -H "Accept:application/json" -H "Content-Type:application/json" ${DEBEZIUM_URL} -d "$JSON_PAYLOAD"
echo -e "\n Hoàn thành đăng ký Analytics Connector"
